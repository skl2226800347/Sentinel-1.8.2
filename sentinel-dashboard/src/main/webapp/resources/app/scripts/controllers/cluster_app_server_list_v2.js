...
app.controller('SentinelClusterAppServerListControllerV2', ['$scope', '$stateParams', 'ngDialog',
    'MachineService', 'ClusterStateServiceV2','AppServiceV2',
    function ($scope, $stateParams, ngDialog, MachineService, ClusterStateService,AppService) {
        ...
        function retrieveClusterAssignInfoOfApp() {
            ClusterStateService.fetchClusterUniversalStateOfApp($scope.appInputModel).success(function (data) {
        ...
        $scope.newServerDialog = () => {
            ...
            serverAssignDialog =$scope.serverAssignDialog = ngDialog.open({
        ...
        function doApplyNewSingleServerAssign() {
            ...
            ClusterStateService.applyClusterSingleServerAssignOfApp($scope.appInputModel, request).success((data) => {
                if (data.code === 0 && data.data) {
        ...
        function generateRequestLimitDataStr(limitData) {
            ...
            for (let i = 0; i < limitData.length; i++) {
                let crl = limitData[i];
                if (crl.namespace === $scope.appInputModel) {
        ...
        function retrieveClusterServerInfo() {
            if(!$scope.appInputModel){
                return;
            }
            ClusterStateService.fetchClusterServerStateOfApp($scope.appInputModel).success(function (data) {
                if (data.code === 0 && data.data) {
                    $scope.loadError = undefined;
        ...
        $scope.retrieveClusterServerInfo=retrieveClusterServerInfo;
        retrieveClusterServerInfo();
        ...
        function apiUnbindServerAssign(ids) {
            ClusterStateService.applyClusterServerBatchUnbind($scope.appInputModel, ids).success((data) => {
                if (data.code === 0 && data.data) {
                    let failedServerSet = data.data.failedServerSet;
                    let failedClientSet = data.data.failedClientSet;
                    if (failedClientSet.length === 0 && failedServerSet.length === 0) {
            ...
            AppService.getAllAppList().success(
                function (data) {
                    if (data.code == 0) {
                        if (data.data) {
                            //$scope.machines = [];
                            $scope.appsInputOptions = [];
                            data.data.forEach(function (item) {
                                $scope.appsInputOptions.push({
                                    text: item.app,
                                    value: item.app
                                });
                            });
                        }
                        if ($scope.appsInputOptions.length > 0) {
                            $scope.appInputModel = $scope.appsInputOptions[0].value;
                        }
                    }else{
                        $scope.appsInputOptions = [];
                    }
        ...
        $scope.$watch('appInputModel', function () {
            if ($scope.appInputModel) {
                retrieveClusterServerInfo();
            }
        });
